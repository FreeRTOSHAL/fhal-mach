 ...
 * Copy from Linux Kernel driver/thermal/ti-soc-thermal/dra752-thermal-data.c
 */
static int dra752_adc_to_temp[DRA752_ADC_END_VALUE - DRA752_ADC_START_VALUE + 1] = {
	/* Index 540 - 549 */
...
TEMP_INIT(am57xx, index) {
	struct temp *temp;
	int32_t ret;
	temp = TEMP_GET_DEV(index);
	if (temp == NULL) {
		return NULL;
	}
 ...
 * read value 
 *
 * workaround for errata i814: Bandgap Temperature Read Dtemp Can Be Corrupted 
 */
static inline uint32_t am57xx_temp_read(struct temp *temp) {
	uint32_t val1;
	uint32_t val2;
	val1 = *(temp->base) & 0x3FF;
	val2 = *(temp->base) & 0x3FF;
	if (val1 == val2) {
		return val1;
	}
	/* read third time */
	return *(temp->base) & 0x3FF;
}

TEMP_GET(am57xx, temp, value, waittime) {
	/* no lock needed! */
	uint32_t v = am57xx_temp_read(temp);
	/* value out of range */
	if (v < DRA752_ADC_START_VALUE || v > DRA752_ADC_END_VALUE) {
		return -1;
	}
	v = dra752_adc_to_temp[v - DRA752_ADC_START_VALUE];
	*value = ((float) v) / 1000.;
	return 0;
}

TEMP_GET_ISR(am57xx, temp, value) {
	/* no lock needed! */
	uint32_t v = am57xx_temp_read(temp);
	/* value out of range */
	if (v < DRA752_ADC_START_VALUE || v > DRA752_ADC_END_VALUE) {
...
struct temp temp_mpu = {
	TEMP_INIT_DEV(am57xx)
	HAL_NAME("AM57xx MPU Temperature Sensor")
	.base = (uint32_t *) 0x6A00232C,
};
TEMP_ADDDEV(am57xx, temp_mpu);

struct temp temp_gpu = {
	TEMP_INIT_DEV(am57xx)
	HAL_NAME("AM57xx GPU Temperature Sensor")
	.base = (uint32_t *) 0x6A002330,
};
TEMP_ADDDEV(am57xx, temp_gpu);

struct temp temp_core = {
	TEMP_INIT_DEV(am57xx)
	HAL_NAME("AM57xx Core Temperature Sensor")
	.base = (uint32_t *) 0x6A002334,
};
TEMP_ADDDEV(am57xx, temp_core);

struct temp temp_iva = {
	TEMP_INIT_DEV(am57xx)
	HAL_NAME("AM57xx IVA Temperature Sensor")
	.base = (uint32_t *) 0x6A002578,
};
TEMP_ADDDEV(am57xx, temp_iva);

struct temp temp_dspeve = {
	TEMP_INIT_DEV(am57xx)
	HAL_NAME("AM57xx DSP/EVE Temperature Sensor")
	.base = (uint32_t *) 0x6A002574,
};
TEMP_ADDDEV(am57xx, temp_dspeve);
